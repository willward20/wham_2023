...
model.load_state_dict(torch.load("STILL_MODEL.pth", map_location=torch.device('cpu')))
...
while True:
    ...
    print(pred)
    steering, throttle = pred[0][0].item(), pred[0][1].item() # fixed so steering and throttle are assigned correrctly
    print("steering: ", steering)
    print("throttle: ", throttle)
    motor.drive(throttle * throttle_lim) # we remove the negative before throttle so it doesn't drive bkwards
